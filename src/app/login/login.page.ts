...
export class LoginPage implements OnInit {
  ...
  login() {
    if (this.email && this.username && this.password) {
      ...
      this.http.get<any[]>(`http://192.168.1.88:3000/usuario?email=${this.email}&username=${this.username}&password=${this.password}`)
        .subscribe({
          next: async (response) => {
  ...
  onSubmit() {
    if (this.email) {
      ...
      this.http.get<any[]>(`http://192.168.1.88:3000/usuario?email=${this.email}`)
        .subscribe({
          next: (response) => {
            if (response.length > 0) {
